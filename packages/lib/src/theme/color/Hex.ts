...
import type { RGBArray } from "./types/RGBArray"
import { RGB } from "./RGB"
...
export class Hex {
	...
	toRGBArray(): RGBArray {
		const red = Number.parseInt(this._hexString.slice(1, 3), 16)
		const green = Number.parseInt(this._hexString.slice(3, 5), 16)
		const blue = Number.parseInt(this._hexString.slice(5, 7), 16)
